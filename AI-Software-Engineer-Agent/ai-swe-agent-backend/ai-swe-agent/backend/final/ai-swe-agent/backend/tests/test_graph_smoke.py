...
def test_environment_error_short_circuits_without_touching_the_reviewer():
    """
    Regression test for a real report: a Docker-unreachable sandbox was being
    routed through the Reviewer, which confidently "diagnosed" the connection
    error as a Python bug and sent the Coder fix instructions for code that
    was never actually broken. An infra_error must end the run immediately —
    the Coder should be called exactly once (the first draft), never asked
    to "fix" anything, and the Reviewer should never run at all.
    """

    class _FakeInfraFailExecutor:
        async def run(self, code, dependencies):
            return ExecutionResult(
                stdout="",
                stderr="[sandbox error] Could not reach the Docker daemon\nIs Docker running?",
                exit_code=1,
                infra_error=True,
            )
    ...
    llms = {
        "planner": ScriptedLLM([PLAN_JSON]),
        "coder": ScriptedLLM([GOOD_CODE]),  # would succeed if it ever got a chance to run
        "reviewer": ScriptedLLM([REVIEW_JSON]),
    }

    with (
        patch.object(nodes_mod, "get_llm", lambda role: llms[role]),
        patch.object(nodes_mod, "get_executor", lambda: _FakeInfraFailExecutor()),
    ):
        graph = build_graph()
    ...
    kinds = [(e["agent"], e["type"]) for e in events]
    assert kinds.count(("coder", "code")) == 1  # never asked for a "fix"
    assert ("reviewer", "review") not in kinds  # never blamed the code
    assert ("system", "failure") not in kinds  # distinct from "gave up debugging"
    assert ("system", "success") not in kinds
    assert ("system", "error") in kinds  # the environment_error terminal event
