...
async def mark_environment_error_node(state: AgentState) -> dict:
    """
    Reached when the sandbox itself couldn't run — Docker unreachable, image
    not built, etc. This is deliberately a *separate* terminal state from
    mark_failure: the code was never actually executed, so looping it back
    through the Reviewer/Coder would just waste calls "fixing" a script that
    was never broken (this is exactly what happens if you skip this check —
    the Reviewer confidently diagnoses a Docker connection error as a Python
    bug, and the Coder dutifully regenerates the same working code).
    """
    iteration = state.get("iteration", 0)
    return {
        ...
        "events": [
            _event(
                ...
                "error",
                (
                    "Sandbox environment isn't available, so nothing was actually run:\n"
                    f"{state.get('stderr', '')}\n\n"
                    "This is a setup problem, not a bug in the generated code — the script "
                    "above hasn't been touched. Check that Docker is running and the sandbox "
                    "image is built (`docker build -t agent-sandbox:latest -f "
                    "Dockerfile.sandbox .`), or set EXECUTOR_BACKEND=subprocess in .env to "
                    "test without Docker."
                ),
                iteration,
            )
