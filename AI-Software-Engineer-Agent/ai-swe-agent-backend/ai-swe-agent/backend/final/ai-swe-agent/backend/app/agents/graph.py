...
Graph shape:
    ...
    START -> planner -> coder -> executor -+
                            ...
                            +----------------------------------------------------------+
                                           ...
                                           |
                                            ------ environment_error (Docker unreachable,
                                                    image missing, etc.) -> mark_environment_error -> END

`coder` is visited once for the first draft (iteration 0) and once more per
debug cycle; `reviewer` is what increments `iteration` and loops back. The
`environment_error` branch is checked first and never touches the debug
loop — an unreachable sandbox isn't a bug in the generated code, so it
shouldn't cost the Reviewer/Coder a wasted round trip diagnosing one.
"""
...
def _route_after_execution(state: AgentState) -> str:
    ...
    if state.get("infra_error"):
        # The sandbox itself failed to run (Docker down, image missing, ...).
        # Checked *before* success/failure so a broken environment never gets
        # routed to the Reviewer as if it were a bug in the generated code.
...
def build_graph():
    ...
    graph.add_node("mark_environment_error", mark_environment_error_node)
    ...
    graph.add_conditional_edges(
        ...
        _route_after_execution,
        {
            ...
            "environment_error": "mark_environment_error",
        },
    )
    ...
    graph.add_edge("mark_environment_error", END)
